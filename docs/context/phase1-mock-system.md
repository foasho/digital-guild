...
しかし地方民はITに疎いという限定で自治体や法人が周辺のお悩みをもとに仕事を作る。
...
今回作成するのは実際の労働者が使うであろうアプリと、労働者不足に悩む人たちの問題を発注したい自治体側のアプリ。

労働者向けのアプリに関しては、旅行に行くような感覚で各地困っている地方のクエストの一覧を見て、日付や作業内容をみて受注し、受注したクエストをStatusチェックや報告などが行えるアプリ。

自治体側の発注者アプリは、労働者不足に悩む人たちの内容をもとにクエストを新規作成する。新規作成されたクエストは、労働者向け掲示板に可視化されるようになる。補助金などがあれば、AIがコインを連動することによって、自動で緊急度の高いものなどはインセンティブをつけてクエストが発注される。
...
workersとrequestersのデータは１つずつで、

UI切り替えによってデータを切り替えるようにしてください。

また、補助金も120万くらいを１つrequetersに紐づけ、インセンティブの割合はモックなので一律で0.5%(MAX依頼値の50%を超えない)としてください。これはジョブが新規作成されたときに、1日1回0時に補助金の情報を確認し更新させるが、モックではこのバッチ処理はさせない。
